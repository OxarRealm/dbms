# SELECT查询处理器测试脚本
# 用途：编译并运行SelectHandler测试程序

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  SELECT Handler Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$testFile = Join-Path $testDir "test_select_handler.cpp"
...
$outputExe = Join-Path $outputDir "test_select_handler.exe"
...
if ($gppCheck) {
    ...
    & g++ -std=c++17 -Wall -I"$projectRoot\include" "$testFile" "$tokenCpp" "$lexerCpp" "$parserCpp" "$astNodeCpp" "$fileManagerCpp" "$tableManagerCpp" "$dataManagerCpp" "$createTableHandlerCpp" "$insertHandlerCpp" "$selectHandlerCpp" -o "$outputExe" 2>&1 | Write-Host
...
$testResult = & .\test_select_handler.exe 2>&1
