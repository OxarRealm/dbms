# INSERT处理器测试脚本
# 用途：编译并运行InsertHandler测试程序

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  INSERT Handler Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$testFile = Join-Path $testDir "test_insert_handler.cpp"
...
$outputExe = Join-Path $outputDir "test_insert_handler.exe"
...
if ($gppCheck) {
    ...
    & g++ -std=c++17 -Wall -I"$projectRoot\include" "$testFile" "$tokenCpp" "$lexerCpp" "$parserCpp" "$astNodeCpp" "$fileManagerCpp" "$tableManagerCpp" "$dataManagerCpp" "$createTableHandlerCpp" "$insertHandlerCpp" -o "$outputExe" 2>&1 | Write-Host
...
$testResult = & .\test_insert_handler.exe 2>&1
