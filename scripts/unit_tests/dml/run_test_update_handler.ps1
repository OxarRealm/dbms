# UPDATE处理器测试脚本
# 用途：编译并运行UpdateHandler测试程序

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  UPDATE Handler Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$testFile = Join-Path $testDir "test_update_handler.cpp"
...
$outputExe = Join-Path $outputDir "test_update_handler.exe"
...
if ($gppCheck) {
    ...
    & g++ -std=c++17 -Wall -I"$projectRoot\include" "$testFile" "$tokenCpp" "$lexerCpp" "$parserCpp" "$astNodeCpp" "$fileManagerCpp" "$tableManagerCpp" "$dataManagerCpp" "$createTableHandlerCpp" "$insertHandlerCpp" "$updateHandlerCpp" -o "$outputExe" 2>&1 | Write-Host
...
$testResult = & .\test_update_handler.exe 2>&1
