# DML执行器测试脚本
# 用途：编译并运行DMLExecutor测试程序

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  DML Executor Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$testFile = Join-Path $testDir "test_dml_executor.cpp"
...
$outputExe = Join-Path $outputDir "test_dml_executor.exe"
...
if ($gppCheck) {
    ...
    & g++ -std=c++17 -Wall -I"$projectRoot\include" "$testFile" "$tokenCpp" "$lexerCpp" "$parserCpp" "$astNodeCpp" "$fileManagerCpp" "$tableManagerCpp" "$dataManagerCpp" "$createTableHandlerCpp" "$insertHandlerCpp" "$deleteHandlerCpp" "$updateHandlerCpp" "$dmlExecutorCpp" -o "$outputExe" 2>&1 | Write-Host
...
$testResult = & .\test_dml_executor.exe 2>&1
