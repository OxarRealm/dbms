# SQL SELECT语句解析器测试脚本
# 用途：编译并运行Parser SELECT解析测试程序

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  SQL SELECT Parser Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

$ErrorActionPreference = "Continue"
$testDir = Split-Path -Parent $MyInvocation.MyCommand.Path
# 从 scripts/unit_tests/sql_parser 回到项目根目录
$projectRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $testDir))
$testFile = Join-Path $testDir "test_parser_select.cpp"
...
$parserSelectCpp = Join-Path $projectRoot "src\sql_parser\parser_select.cpp"
$parserWhereCpp = Join-Path $projectRoot "src\sql_parser\parser_where.cpp"
$astNodeCpp = Join-Path $projectRoot "src\sql_parser\ast_node.cpp"
$outputDir = Join-Path $projectRoot "test_results\unit_tests"
$outputExe = Join-Path $outputDir "test_parser_select.exe"
...
if ($gppCheck) {
    ...
    & g++ -std=c++17 -Wall -I"$projectRoot\include" "$testFile" "$tokenCpp" "$lexerCpp" "$parserCpp" "$parserSelectCpp" "$parserWhereCpp" "$astNodeCpp" -o "$outputExe" 2>&1 | Write-Host
...
$testResult = & .\test_parser_select.exe 2>&1
