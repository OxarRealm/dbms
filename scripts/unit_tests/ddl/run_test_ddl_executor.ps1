# DDL执行器测试脚本
# 用途：编译并运行DDLExecutor测试程序

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  DDL Executor Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$testFile = Join-Path $testDir "test_ddl_executor.cpp"
...
$outputExe = Join-Path $outputDir "test_ddl_executor.exe"
...
if ($gppCheck) {
    ...
    & g++ -std=c++17 -Wall -I"$projectRoot\include" "$testFile" "$tokenCpp" "$lexerCpp" "$parserCpp" "$astNodeCpp" "$tableManagerCpp" "$dataManagerCpp" "$createTableHandlerCpp" "$editTableHandlerCpp" "$renameTableHandlerCpp" "$dropTableHandlerCpp" "$ddlExecutorCpp" -o "$outputExe" 2>&1 | Write-Host
...
$testResult = & .\test_ddl_executor.exe 2>&1
