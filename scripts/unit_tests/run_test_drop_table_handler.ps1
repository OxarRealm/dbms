# DROP TABLE处理器测试脚本
# 用途：编译并运行DropTableHandler测试程序

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  DROP TABLE Handler Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$testFile = Join-Path $testDir "test_drop_table_handler.cpp"
...
$outputExe = Join-Path $outputDir "test_drop_table_handler.exe"
...
if ($gppCheck) {
    ...
    & g++ -std=c++17 -Wall -I"$projectRoot\include" "$testFile" "$tokenCpp" "$lexerCpp" "$parserCpp" "$astNodeCpp" "$tableManagerCpp" "$dataManagerCpp" "$createTableHandlerCpp" "$dropTableHandlerCpp" -o "$outputExe" 2>&1 | Write-Host
...
$testResult = & .\test_drop_table_handler.exe 2>&1
