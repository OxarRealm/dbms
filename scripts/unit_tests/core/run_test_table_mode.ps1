# TableMode数据结构测试脚本
# 用途：编译并运行TableMode测试程序

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  TableMode Data Structure Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$testFile = Join-Path $testDir "test_table_mode.cpp"
$outputDir = Join-Path $projectRoot "test_results\unit_tests"
$outputExe = Join-Path $outputDir "test_table_mode.exe"
...
if ($gppCheck) {
    ...
    $compileCmd = "g++ -std=c++17 -Wall -I`"$projectRoot\include`" `"$testFile`" -o `"$outputExe`""
    & g++ -std=c++17 -Wall -I"$projectRoot\include" "$testFile" -o "$outputExe" 2>&1 | Write-Host
...
$testResult = & .\test_table_mode.exe 2>&1
