...
// CREATE INDEX和DROP INDEX解析方法实现

std::unique_ptr<ASTNode> Parser::parseCreateIndex() {
    // CREATE INDEX [index_name] ON table_name(field_name) USING index_type IN database_name;
    auto node = std::make_unique<CreateIndexNode>();
    
    // CREATE
    if (!expect(TokenType::CREATE, "CREATE")) return nullptr;
    // INDEX
    if (!expect(TokenType::INDEX, "INDEX")) return nullptr;
    
    // 可选的索引名称（暂时不支持命名索引，索引名由系统自动生成）
    // 如果下一个token是ON，说明没有索引名
    // 如果下一个token是IDENTIFIER，再检查下一个是否是ON
    if (m_currentToken.type == TokenType::IDENTIFIER) {
        Token savedToken = m_currentToken;
        advance();
        if (m_currentToken.type == TokenType::ON) {
            // 这是索引名，保存它（虽然目前不使用）
            node->indexName = savedToken.value;
        } else {
            // 这不是索引名，重新解析（这种情况不应该发生）
            setError("Unexpected token after INDEX");
            return nullptr;
        }
    }
    ...
    // table_name
    node->tableName = parseIdentifier();
    if (node->tableName.empty()) return nullptr;
    
    // (
    if (!expect(TokenType::LEFT_PAREN, "(")) return nullptr;
    
    // field_name
    node->fieldName = parseIdentifier();
    if (node->fieldName.empty()) return nullptr;
    
    // )
    if (!expect(TokenType::RIGHT_PAREN, ")")) return nullptr;
    
    // USING index_type (可选，默认�?hash")
    if (m_currentToken.type == TokenType::USING) {
        advance();  // 跳过USING
        if (m_currentToken.type == TokenType::IDENTIFIER) {
            std::string indexType = m_currentToken.value;
            // 转换为小�?
            for (char& c : indexType) {
                if (c >= 'A' && c <= 'Z') {
                    c = c - 'A' + 'a';
                }
            }
            if (indexType == "hash" || indexType == "adjacent" || indexType == "btree") {
            ...
            setError("Expected index type after USING");
            return nullptr;
        }
    } else {
        // 默认使用hash索引
...
std::unique_ptr<ASTNode> Parser::parseDropIndex() {
    // DROP INDEX [index_name] ON table_name(field_name) [USING index_type] IN database_name;
    auto node = std::make_unique<DropIndexNode>();
