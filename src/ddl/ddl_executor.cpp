...
bool DDLExecutor::execute(const std::string& sql) {
    ...
    switch (type) {
        ...
        case StatementType::CREATE_INDEX:
            result = m_createIndexHandler.execute(sql, m_databasePath);
            if (!result) {
                setError("CREATE INDEX execution failed: " + m_createIndexHandler.getLastError());
            }
            break;
        ...
        case StatementType::DROP_INDEX:
            result = m_dropIndexHandler.execute(sql, m_databasePath);
            if (!result) {
                setError("DROP INDEX execution failed: " + m_dropIndexHandler.getLastError());
            }
            break;
