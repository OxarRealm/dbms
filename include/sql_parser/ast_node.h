...
/**
 * @brief CREATE INDEX语句AST节点
 */
class CreateIndexNode : public ASTNode {
public:
    std::string indexName;             // 索引名称（可选，如果没有指定则自动生成）
    std::string tableName;             // 表名
    ...
    void accept(ASTVisitor* visitor) override;
    std::string getNodeType() const override { return "CreateIndexNode"; }
};

/**
 * @brief DROP INDEX语句AST节点
 */
class DropIndexNode : public ASTNode {
public:
    std::string indexName;             // 索引名称（可选，如果指定索引名）
    std::string tableName;             // 表名（可选，如果通过表名和字段名指定）
    std::string fieldName;             // 字段名（可选，如果通过表名和字段名指定）
    std::string indexType;             // 索引类型（可选，如果指定类型）
    std::string databaseFileName;      // 数据库文件名
    
    DropIndexNode() {}
    
    void accept(ASTVisitor* visitor) override;
    std::string getNodeType() const override { return "DropIndexNode"; }
};
